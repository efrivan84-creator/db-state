...
- Docs and demo. The README explains how db-state differs from Meteor, Hasura, Firebase/Supabase, Convex and local-first sync, where it fits best and where its boundary is. `demo/` shows `readChange` (order notes visible to whoever may edit the order) and merged permissions (`order.number` from a separate group, a `viewer` user); `demo:smoke` checks it over WebSocket and now runs in `npm test`. The demo in-memory Mongo understands `$and`/`$or`/`$in`, which the library builds itself when applying permissions. `@db-state/core` and `@db-state/vue` ship at the same version with no code changes.
