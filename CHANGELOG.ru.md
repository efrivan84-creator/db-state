...
- `info.editid` / `info.editdata` больше не пишутся в `change.set` при обновлении. Они повторяли то, что запись журнала уже сообщает через `userId` и `createdAt`, а клиент применял их к своему кэшу при каждой синхронизации. В документах `info` остаётся — там оно читается вместе с записью; дублировать его перестал только журнал. `insert` и `delete` не затронуты: их `obj` / `old` — полные снимки документа, где `info` является частью документа, а не повторённым полем журнала.
