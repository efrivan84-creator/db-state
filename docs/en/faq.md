...
`0.0.x`. The shape of the API is stable and 38 tests cover the core behavior, but you should expect minor breaking changes until `1.0`. Audit trail and time-travel work by design; the lack of optimistic concurrency control is intentional (see [next question](#what-about-conflicts-between-concurrent-edits)).
