...
System events use different shapes (e.g. RPC has `id`, `method`, `payload` fields). The client routes `dbstate:rpc_result` / `dbstate:rpc_error` internally, but also emits those envelopes to `state.socket.on(...)` for diagnostics. This is useful when the server sends metadata such as `meta.fieldsFiltered`.
