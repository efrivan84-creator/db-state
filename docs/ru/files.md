...
Одинаковые файлы хранятся один раз, а повторная загрузка не передаёт байты.

1. Клиент (`@db-state/vue-files`) перед загрузкой считает SHA-256 файла и
   отправляет его в `upload_start`.
2. Если на сервере уже есть готовый файл с таким хэшем и размером, байты не
   запрашиваются: создаётся новая строка `file` со своим `ownerId`, `token` и
   `downloadPolicy`, указывающая на тот же объект хранилища. Ответ
   `upload_done` приходит сразу, с `deduplicated: true`.
3. Иначе загрузка идёт как обычно, а сервер сам считает SHA-256 по пришедшим
   байтам. Заявленный хэш не совпал — загрузка отклоняется (`File checksum
   mismatch`), файл не сохраняется. Совпал бинарь с уже лежащим — вторая копия
   на диске не хранится, строка ссылается на прежний объект.

Хэш от клиента — только ключ поиска: в базу пишется хэш, посчитанный сервером.
Поэтому подложить чужой хэш под другие байты нельзя.

**Цена.** Кто знает SHA-256 файла, лежащего на сервере, получает этот файл:
заявляет хэш и размер — и получает свой `token`. Хэш становится таким же ключом,
как `token`, поэтому `sha256` не отдаётся клиенту никогда. Если такой риск
неприемлем — `dedupe: false` на сервере.

Настройки:

| Где | Параметр | По умолчанию | Значение |
|---|---|---|---|
| сервер | `dedupe` | `true` | `false` — без поиска по хэшу и без склейки одинаковых бинарей |
| клиент | `upload(file, { hash })` | `true` | `false` — не считать хэш для этой загрузки |
| клиент | `createFileClient(state, { hashMaxSize })` | 256 МБ | файлы больше грузятся без хэша: `crypto.subtle` считает хэш целиком в памяти |

Хэш считается только там, где есть `crypto.subtle` (https или localhost).

Один объект хранилища может принадлежать нескольким строкам. Удаления в API
пока нет; будущее удаление должно стирать объект, только когда на него не
ссылается ни одна строка.
...
Кто видит строки `file`, решают **права группы**, как у любой таблицы. Модуль
только защищает саму таблицу:

- `storageKey` и `sha256` не уходят клиенту никогда — ни в `load`/`getIds`, ни
  в `sync` (хук модуля `readChange`), ни в ответе на загрузку;
- `token` появляется только у готового файла;
- прямые write-операции запрещены, кроме внутренних вызовов файлового модуля.

Личный список файлов — фильтр права по владельцу:

```js
{ file: { read: { ownerId: "$adminid" } } }
```

В строке лежит `token` — ключ на скачивание. Кому видна чужая строка, тот
скачает чужой файл (если пускает политика). Если строки должны быть видны
только владельцу при любых правах, включая `fullaccess`, допишите в приложении
табличные `beforeRead` и `readChange` для таблицы файлов.
...
| `dedupe` | `true` | Дедупликация по SHA-256, см. выше. |
...
| `hashMaxSize` | 256 МБ | Файлы больше грузятся без хэша. |
...
Только свои файлы список вернёт при праве с фильтром по владельцу: `{ file: { read: { ownerId: "$adminid" } } }` (см. «Доступ к `state.file`»).
