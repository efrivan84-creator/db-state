...
Проверь, пришел ли `changes_available`, прошел ли `syncNow()`, и есть ли read permission на таблицу. Если сервер скрывает rows, в RPC envelope может быть `meta.accessFiltered`.
