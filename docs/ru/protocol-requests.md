...
- `read_fields` / `write_fields` ограничивают набор полей. При чтении лишние поля не приходят, при записи запрос с чужим полем **отклоняется целиком** с ошибкой `Write denied: field <путь>`. Пустой список оставляет только `_id` при чтении и не разрешает ни одного клиентского поля при записи.
- `read_fields` ограничивает и `filter`, и `getIds.sort`: скрытое поле отклоняется с `Read denied: field <путь>` — иначе значение подбиралось бы по наличию или порядку строк. Полевые операторы вроде `$regex` разрешены для видимых полей, а непрозрачные корневые операторы (`$expr`, `$where`, `$text`, `$jsonSchema` и подобные) при активном списке полей отклоняются.
