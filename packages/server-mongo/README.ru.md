...
- Каждый файловый метод по умолчанию получает `db` (Mongo этого сервера), `api` (сам db-state сервер: `api.add`/`api.update` пишут с логом и broadcast) и `user` (из `client.user`). Нужно больше — `methodsContext: {...}` добавляет своё поверх (одноимённые ключи переопределяют дефолты). Хуки получают те же дополнительные ключи, но их `db` и `api` всегда остаются серверными; эта защита относится к `ctx` хука, а не к слиянию контекста файлового метода.
...
### Методы без входа: папка `pub`

Обычный RPC требует вошедшего клиента. Регистрация и восстановление пароля
так не выражаются: чтобы позвать метод, нужна сессия, — а её у человека как
раз и нет.

Поэтому файлы в подпапке `pub/` вызываются без входа:

```
rpc/
  pub/
    auth/
      register.js   → "pub.auth.register", вход не нужен
      reset.js      → "pub.auth.reset",    вход не нужен
  zad/
    get-num.js      → "zad.get-num",       вход обязателен
```

```js
// rpc/pub/auth/reset.js
export default async ({ body, db }) => {
  // user сюда не приходит: проверять вход нечему, метод отвечает за себя сам.
...
- Признак — **папка**, а не список в конфиге и не префикс в имени файла:
  «что открыто наружу» видно командой `ls rpc/pub`, и файл либо лежит в ней,
  либо нет — опечатке негде спрятаться.
- Имя метода у клиента несёт ту же метку (`pub.`), поэтому открытый вызов
  видно и в браузере, и в логах.
- Публичный метод **не получает `user`** и проверяет вход сам. Ограничение
  попыток, одноразовые коды, проверка данных — на самом методе.
- Табличные команды (`load`, `getIds`, `update`, `sync`) публичными не
  становятся никогда: они читают и пишут данные. Без `methodsDir` публичных
  методов нет вовсе.
...
в any-of, `{}` (все строки) шире любого фильтра. Сливаются все действия, а не
только `read` и `write`: полномочия именованных методов (`bill: { pay: {} }`)
переживают вход и проверяются `accessAllows`.
...
`sync` проверяет каждое изменение правом группы на его таблицу. Табличный
`beforeRead` на изменения не вызывается: он сужает `ctx.filter` и разрешает
запрос, а у изменения фильтра нет. Правило строки, которое фильтром не
выразить, — членство в другой коллекции, право на родительский документ, —
пишется в `readChange`. Он вызывается на каждое изменение с `table`, `id`,
`change` и `loadDoc()`:
...
export default async (ctx) => {
  ...
  if (!await isMember(ctx.db, row.box, ctx.user)) return false  // не отдаём
  ctx.fields = ["box", "subject"]          // урезаем поля
  return true                              // без права группы
}
```

`false` не отдаёт изменение, `true` отдаёт без права группы, отсутствие решения
оставляет право группы; `ctx.fields` пересекается с `read_fields`. `fullaccess`
хук не обходит. Без файла `sync` работает как раньше.
...
Клиенты вызывают `sync({ from, sessionId })`. Сервер читает не более 12 часов журнала, исключает session отправителя, применяет хуки `readChange` и права на чтение, фильтрует запрещённые поля и возвращает `{ to, changes, hasMore? }`. Клиент автоматически проходит все окна с `hasMore`.
