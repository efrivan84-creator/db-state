...
Объект query уходит на сервер как `{ table, ...query }`, так что `filter`, `sort`, `skip`, `limit` поддерживаются тем же API. При создании ref сначала читает последние закэшированные id из IndexedDB/кэша и сразу сервер не дёргает. ids-ref обновляется после ручного логина и после изменений таблицы, потом сохраняется в кэш. Hash-аут и обновление страницы его не обновляют. Повторный запрос того же query для той же таблицы вернёт существующий ref.
...
```text
countRef/idsRef создан -> читаем кэшированное значение -> ждём логин или изменение таблицы -> refresh с сервера -> сохраняем в кэш
```
...
Когда сокет открывается после обновления страницы, клиент читает сохранённые `userId/hash`, вызывает `authByHash`, потом запускает sync. `countRef`/`idsRef` он не обновляет, пока sync не вернёт изменений по таблицам. Если сервер отклонил сохранённый hash, клиент очищает auth-данные и переходит в анонимное состояние.
