...
- Cache-first query-ref'ы: закэшированные ids/counts сразу рисуются из IndexedDB, потом обновляются после логина или synced изменений своей таблицы.
...
Сервер всё равно принимает решение о доступе через `access` групп и файловые хуки.
...
При создании ref сначала читает последнее закэшированное значение из IndexedDB/кэша и сразу сервер не дёргает. Count обновляется после ручного логина, после локальных записей и когда sync применяет изменения этой же таблицы. Сама hash-аутентификация не обновляет все закэшированные count'ы. Обновленное значение записывается обратно в кэш. Если запросить тот же `countRef` повторно с теми же таблицей и фильтром — вернётся существующий ref.
...
Объект query уходит на сервер как `{ table, ...query }`, так что `filter`, `sort`, `skip`, `limit` поддерживаются тем же API. При создании ref сначала читает последние закэшированные id из IndexedDB/кэша и сразу сервер не дёргает. ids-ref обновляется после ручного логина, после локальных записей и когда sync применяет изменения этой же таблицы. Сама hash-аутентификация не обновляет все закэшированные ids-ref'ы. Обновленное значение сохраняется в кэш. Повторный запрос того же query для той же таблицы вернёт существующий ref.
...
```text
countRef/idsRef создан -> читаем кэшированное значение -> ждем логин, пустой кэш или изменение таблицы -> refresh с сервера -> сохраняем в кэш
```
...
Сервер возвращает `userId` и `hash`. Клиент очищает локальный кэш/in-memory таблицы, ставит `time1` в текущий момент логина, сохраняет авторизацию в `localStorage` и перезапрашивает активные реактивные чтения. `login()` не запускает `syncNow()`.
...
Когда сокет открывается после обновления страницы, клиент читает сохранённые `userId/hash`, вызывает `authByHash`, запускает sync и перезапрашивает реактивные документы или query-ref'ы, которые не загрузились из кэша/auth. Кэшированные query-ref'ы всё равно сразу показываются из IndexedDB. Они обновляются только если sync применил изменения их таблицы. Если сервер отклонил сохранённый hash, клиент очищает auth-данные и переходит в анонимное состояние.
