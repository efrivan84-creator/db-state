...
Для динамических правил, которые нельзя выразить фильтром, используйте файлы с `export default` в `hooksDir`. Хук может проверить запрос, пользователя и таблицу, переписать CRUD-запрос, сузить поля или явно разрешить/запретить операцию. Хук `sync` оборачивает весь sync-запрос; видимость каждого изменения по-прежнему определяет декларативный `user.access`.
